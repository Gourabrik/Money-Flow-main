...
// ── Helper: get current Firebase user uid (set by firebase-auth-guard.js) ────
function _uid() { return window._firebaseUser ? window._firebaseUser.uid : null; }
...
// Firebase Auth now handles session state (see firebase-auth-guard.js)
...
async function _migrateLocalToCloud() {
    const uid = _uid();
    if (!uid || !window.FS) return;
    try {
        // Migrate expenses
        for (const exp of expenses) {
            if (!exp._fsId) {
                const fsId = await window.FS.saveExpense(uid, exp);
                exp._fsId = fsId;
            }
        }
        localStorage.setItem('expenses', JSON.stringify(expenses));
        ...
        const ih = JSON.parse(localStorage.getItem('incomeHistory')) || [];
        for (const inc of ih) {
            if (!inc._fsId) await window.FS.saveIncome(uid, inc);
        }

        // Migrate debts
        for (const d of debts) {
            if (!d._fsId) {
                const fsId = await window.FS.saveDebt(uid, d);
                d._fsId = fsId;
            }
        }
        localStorage.setItem('debts', JSON.stringify(debts));

        // Migrate savings history
        const sh = JSON.parse(localStorage.getItem('savingsHistory')) || [];
        for (const s of sh) {
            if (!s._fsId) await window.FS.saveSavingsEntry(uid, s);
        }
        ...
        console.log('[Migrate] localStorage data migrated to Firestore successfully.');
    } catch (err) {
...
function setupEventListeners() {
    ...
    // Logout is handled by firebase-auth-guard.js on the navbar button
    ...
    if (elements.resetDataBtn) elements.resetDataBtn.addEventListener('click', resetData);
...
function handleExpenseSubmit(e) {
    ...
    try {
        ...
        const uid = _uid();
        if (uid && window.FS) {
            window.FS.saveExpense(uid, expense).then(fsId => {
                if (fsId) expense._fsId = fsId; // attach Firestore ID for future deletes
            });
        }
...
function handleIncomeSubmit(e) {
    ...
    try {
        ...
        incomeHistory.push(newIncomeRecord);
        localStorage.setItem('incomeHistory', JSON.stringify(incomeHistory));

        // ── Sync to Firestore in the background ──────────────────────────────
        const uid = _uid();
        if (uid && window.FS) {
            window.FS.saveIncome(uid, newIncomeRecord);
            window.FS.saveUserTotals(uid, { income, onlineIncome, cashIncome });
        }
...
function updateDailyReport() {
    ...
    const container = elements.dailyExpensesList;
    if (!container) return;

    if (todayExpenses.length === 0) {
        container.innerHTML = `<div style="grid-column:span 4;text-align:center;padding:40px 0;opacity:0.5;">
            <i class="fas fa-receipt fa-2x mb-2"></i>
            <p style="font-size:12px;">No expenses today</p>
        </div>`;
    } else {
        const catTotals = {};
        ...
        const totalDay = todayExpenses.reduce((s, e) => s + e.amount, 0);
        container.innerHTML = Object.entries(catTotals)
            .sort(([, a], [, b]) => b - a)
            .map(([cat, total]) => renderCategoryCard(cat, total, totalDay > 0 ? ((total/totalDay)*100).toFixed(0) : 0))
            .join('');
    }
...
function updateMonthlyReport() {
    ...
    if (Object.keys(categoryTotals).length === 0) {
        container.innerHTML = `<div style="grid-column:span 4;text-align:center;padding:40px 0;opacity:0.5;">
            ...
            <p style="font-size:12px;">No expenses this month</p>
        </div>`;
    } else {
        const totalMonthly = monthlyExpenses.reduce((sum, exp) => sum + exp.amount, 0);
        container.innerHTML = Object.entries(categoryTotals)
            .sort(([, a], [, b]) => b - a)
            .map(([cat, total]) => renderCategoryCard(cat, total, ((total/totalMonthly)*100).toFixed(0)))
            .join('');
    }
...
function getAllTransactions() {
    ...
    // Add expense transactions (attach _txKey for reliable deletion)
    expenses.forEach((expense, i) => {
        if (expense.date && expense.amount) {
            transactions.push({
                ...
                _txKey:      'expense|' + i + '|' + expense.date + '|' + expense.amount
            });
        }
    });
    ...
    incomeHistory.forEach((income, i) => {
        if (income.date && income.amount) {
            transactions.push({
                ...
                _txKey:      'income|' + i + '|' + income.date + '|' + income.amount
            });
        }
    });
...
function renderHistoryTable() {
    ...
    // Generate table rows — pass _txKey to deleteTransaction for reliable lookup
    const tableRows = displayTransactions.map((transaction) => {
        ...
        const safeKey     = encodeURIComponent(transaction._txKey || '');
...
function deleteTransactionByKey(encodedKey) {
    ...
    if (type === 'expense') {
        ...
        if (idx > -1) {
            ...
            localStorage.setItem('expenses', JSON.stringify(expenses));
            if (uid && window.FS && deleted._fsId) window.FS.deleteExpense(uid, deleted._fsId);
        }
    } else if (type === 'income') {
        const date   = parts[2];
        const amount = parseFloat(parts[3]);
        ...
        if (idx > -1) {
            ...
            localStorage.setItem('income', income.toString());
            if (uid && window.FS) {
                if (deleted._fsId) window.FS.deleteIncome(uid, deleted._fsId);
                window.FS.saveUserTotals(uid, { income, onlineIncome, cashIncome });
            }
        }
        ...
        if (idx > -1) {
            ...
            if (uid && window.FS) {
                if (deleted._fsId) window.FS.deleteSavingsEntry(uid, deleted._fsId);
                window.FS.saveUserTotals(uid, { savings });
            }
        }
    }

    // Re-render history and update UI
    updatePriceHistoryChart();
    updateUI();
}

// Delete transaction function (legacy — kept for backward compat)
function deleteTransaction(index, type, date, amount) {
    if (confirm('Delete this transaction?')) {
        ...
        if (type === 'expense') {
            ...
            if (expenseIndex > -1) {
                const deleted = expenses.splice(expenseIndex, 1)[0];
                localStorage.setItem('expenses', JSON.stringify(expenses));
            ...
            if (incomeIndex > -1) {
                const deletedIncome = incomeHistory.splice(incomeIndex, 1)[0];
                localStorage.setItem('incomeHistory', JSON.stringify(incomeHistory));
                // ── Sync delete to Firestore ──────────────────────────────────
                if (uid && window.FS && deletedIncome._fsId) window.FS.deleteIncome(uid, deletedIncome._fsId);
                ...
                income = onlineIncome + cashIncome;
                localStorage.setItem('income', income.toString());
                if (uid && window.FS) window.FS.saveUserTotals(uid, { income, onlineIncome, cashIncome });
            }
...
function handleAddDebt() {
    ...
    const uid = _uid();
    if (uid && window.FS) {
        window.FS.saveDebt(uid, newDebt).then(fsId => {
            if (fsId) newDebt._fsId = fsId;
        });
    }
...
function handlePayDebt(debtId, paymentAmount) {
    ...
    // ── Sync to Firestore in the background ──────────────────────────────────
    const uid = _uid();
    if (uid && window.FS && debt._fsId) window.FS.updateDebt(uid, debt._fsId, { amount: debt.amount });
...
function deleteDebt(debtId) {
    if (confirm('Are you sure you want to delete this debt?')) {
        const toDelete = debts.find(d => d.id === debtId);
        debts = debts.filter(debt => debt.id !== debtId);
        localStorage.setItem('debts', JSON.stringify(debts));

        // ── Sync delete to Firestore ──────────────────────────────────────────
        const uid = _uid();
        if (uid && window.FS && toDelete && toDelete._fsId) window.FS.deleteDebt(uid, toDelete._fsId);
